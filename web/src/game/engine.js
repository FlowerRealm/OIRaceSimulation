...
function getSimpleStamina(lv){if(!easyMode)return null;if(lv===4)return 35;if(lv>=5&&lv<=10)return 60;if(lv>=11)return 75;return null}
const LV=[{name:'CSP-S模拟赛',maps:4,pass:200,stam:25,sim:!0},{name:'CSP-S',maps:4,pass:200,stam:25,sim:!1},{name:'NOIP模拟赛1',maps:4,pass:200,stam:35,sim:!0},{name:'NOIP模拟赛2',maps:4,pass:200,stam:35,sim:!0},{name:'NOIP',maps:4,pass:200,stam:35,sim:!1},{name:'省选模拟赛1',maps:6,pass:400,stam:45,sim:!0},{name:'省选模拟赛2',maps:6,pass:400,stam:45,sim:!0},{name:'省选',maps:6,pass:400,stam:45,sim:!1},{name:'NOI模拟赛1',maps:6,pass:400,stam:45,sim:!0},{name:'NOI模拟赛2',maps:6,pass:400,stam:45,sim:!0},{name:'NOI',maps:6,pass:400,stam:45,sim:!1},{name:'CTT',maps:6,pass:400,stam:45,sim:!1},{name:'CTS',maps:6,pass:400,stam:45,sim:!1},{name:'IOI',maps:6,pass:0,stam:45,sim:!1}];
...
const stocks=[{name:'⏰ 时间存量',count:timeStock,cost:stockCosts.time,key:'time',desc:ch.I_exp?'初始时间+2.5':'初始时间+3'},{name:'🧠 思维存量',count:mindStock,cost:stockCosts.mind,key:'mind',desc:'思维永久+1'},{name:'💠 提升存量',count:boostStock,cost:stockCosts.boost,key:'boost',desc:'特殊节点技能点+1'},{name:'⚡ 振奋存量',count:inspireStock,cost:stockCosts.inspire,key:'inspire',desc:'正确时恢复更多精力'}];
...
cont.innerHTML=html}
window._buyStock=function(key){const isProvinceOrLater=level>=4;const priceHike=(ch.VI&&hasPassedNOIP())?1:0;let costs=isProvinceOrLater?{time:5+priceHike,mind:5+priceHike,boost:7+priceHike,inspire:8+priceHike}:{time:3,mind:3,boost:4,inspire:4};const cost=costs[key];if(sp<cost){setGM('技能点不足','warning');return}if(key==='time'&&timeStock<1){setGM('时间存量不足','warning');return}if(key==='mind'&&mindStock<1){setGM('思维存量不足','warning');return}if(key==='boost'&&boostStock<1){setGM('提升存量不足','warning');return}if(key==='inspire'&&inspireStock<1){setGM('振奋存量不足','warning');return}sp-=cost;if(key==='time'){timeStock--;const bonus=ch.I_exp?2.5:3;timeBonus+=bonus;setGM('⏰时间升级！初始时间+'+bonus,'success')}else if(key==='mind'){mindStock--;mindBonus++;setGM('🧠思维升级！思维+1','success')}else if(key==='boost'){boostStock--;boostLevel++;setGM('💠提升升级！特殊节点奖励+1','success')}else if(key==='inspire'){inspireStock--;inspireCount++;setGM('⚡振奋升级！','success')}everBoughtStock=true;syncAllSp();updateSideStocks();updateActBar();initShopPanel();updateShopPreview();refreshUI();checkAchievements()}
