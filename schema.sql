-- OI Race Simulation user system schema.
--
-- Design note: the username is the only identity. The old client-side
-- leaderboard keyed players by a free-text display name and had to append
-- " #1", " #2", ... whenever two players picked the same one. A UNIQUE column
-- makes that whole collision-avoidance loop disappear.
